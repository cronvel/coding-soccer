...
Renderer.create = function( options ) {
	...
	self.posts.push( GoalPost.create( { x: - this.$fieldWidth / 2 , y: - ( 0.06 + 7.32 / 2 ) } ) ) ;
	self.posts.push( GoalPost.create( { x: - this.$fieldWidth / 2 , y: + ( 0.06 + 7.32 / 2 ) } ) ) ;
	self.posts.push( GoalPost.create( { x: this.$fieldWidth / 2 , y: - ( 0.06 + 7.32 / 2 ) } ) ) ;
	self.posts.push( GoalPost.create( { x: this.$fieldWidth / 2 , y: + ( 0.06 + 7.32 / 2 ) } ) ) ;
...
Renderer.prototype.coord = function( obj ) {
	return {
		...
		z: Math.round( Math.max( 0 , ( obj.z * this.pixelRatio ) ) )
	} ;
} ;
...
function GoalPost( x , y ) { return Player.create( x , y ) ; }

GoalPost.create = function( x , y ) {
