...
Renderer.create = function( options ) {
	...
	/*
	self.posts.push( GoalPost.create( self.coord( { x: - self.fieldWidth / 2 , y: - ( 0.06 + 7.32 / 2 ) } ) ) ) ;
	self.posts.push( GoalPost.create( self.coord( { x: - self.fieldWidth / 2 , y: + ( 0.06 + 7.32 / 2 ) } ) ) ) ;
	self.posts.push( GoalPost.create( self.coord( { x: self.fieldWidth / 2 , y: - ( 0.06 + 7.32 / 2 ) } ) ) ) ;
	self.posts.push( GoalPost.create( self.coord( { x: self.fieldWidth / 2 , y: + ( 0.06 + 7.32 / 2 ) } ) ) ) ;
	*/
...
Renderer.prototype.init = function() {
	...
	this.halfTimeDuration = this.game.parameters.halfTimeDuration ;
	this.fullDurationMs = 2000 * this.halfTimeDuration ;
	this.maxFrame = this.fullDurationMs / this.engineFramePeriod ;
	
	var svgField = SvgField.create( this.game.parameters.field , this.pixelRatio ) ;
	...
	this.$fieldWidth = this.game.parameters.field.outerLength * this.pixelRatio ;
	this.$fieldHeight = this.game.parameters.field.outerWidth * this.pixelRatio ;
